...
# PG17 IS THE FLOOR AND EVERY ARM BELOW IS VACUOUS WITHOUT IT (#1264).
# PostgreSQL 15 and 16 refuse an access method on a partitioned table outright,
# so `ALTER TABLE ... SET ACCESS METHOD pgcolumnar` does nothing there and the
# fixture never exists. Measured on 15.18: the ALTER is refused and relam comes
# back empty. Every arm then fails on a correct build -- which is what the
# 2026-09-25 nightly showed on `suites (PG 15, x86_64)` and `(PG 16, x86_64)`,
# while the per-PR gate runs only 17 and 18 and was green.
#
# The same floor and the same reason as inheritance.sh's arms (#1261). Skipped
# BY NAME rather than left out, so a reader sees which properties were not
# measured on this major instead of a shorter list.
if [ "${PGC_MAJOR:-0}" -lt 17 ]; then
	for _pq_arm in \
		"premise: the parent is partitioned and carries the columnar access method" \
		"premise: so it has no storage of its own" \
		"premise: a parquet file exists, so the import reaches the relation" \
		"control: importing that file into the LEAF still works" \
		"control: a HEAP target still imports, so the guard is about storage" \
		"importing into the partitioned parent is refused" \
		"and the cluster is still up, so the refusal was not an abort"; do
		check_skip "$_pq_arm" \
			"SKIP  $_pq_arm (a partitioned table cannot carry an access method before PG17; this server is ${PGC_MAJOR:-unknown})" \
